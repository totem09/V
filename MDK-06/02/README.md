# Полный Список Экзаменационных Билетов МДК.06.02  
**«ИНЖЕНЕРНО-ТЕХНИЧЕСКАЯ ПОДДЕРЖКА СОПРОВОЖДЕНИЯ ИНФОРМАЦИОННЫХ СИСТЕМ»**

Ниже приведён **27 билетов**. Каждый билет содержит три основных вопроса:  
1) Теоретическая часть (задачи сопровождения, реинжиниринг, методы резервного копирования и пр.),  
2) Практические/организационные аспекты (договор, сценарий, регламенты, мониторинг и т. д.),  
3) Анализ сопровождения объекта информации (характер угроз, каналы, причины нарушения целостности и возможные ошибки совместимости).  

Для каждого вопроса даётся **короткий ответ** и **подробный ответ**.
...
### Вопрос 1. Восстановление информации в информационной системе.

**Краткий ответ:**
- Использование резервных копий, план аварийного восстановления (**DRP**), тестирование процедур.

**Подробный ответ:**
Чтобы не терять данные при сбоях, важно иметь дублирующие копии и знать процедуру восстановления шаг за шагом: где хранится бэкап, как быстро его разворачивать, кто ответственный. Регулярные тренировки (**drill**) позволяют убедиться, что восстановление займёт приемлемое время, а все нужные версии файлов доступны.

### Вопрос 2. Цели и регламенты резервного копирования.

**Краткий ответ:**
- **Цели:** сократить риск потери данных, обеспечить минимизацию простоя.
- **Регламенты:** периодичность, место хранения, порядок проверки бэкапов.

**Подробный ответ:**
Резервное копирование важно планировать: какие данные копируем (**весь объём** или только **критичные**), где храним (**на локальном NAS**, **в облаке**), как часто (**ежедневно**, **еженедельно**), сколько копий сохраняем (**принцип «GFS»** — grandfather-father-son). Регламенты определяют, кто отвечает за запуск бэкапов, кто проверяет целостность и кто имеет доступ к архивам.

### Вопрос 3. [Анализ сопровождения объекта (компьютерная сеть материальной группы)](#билет-5-вопрос-3-анализ-сопровождения-объекта-компьютерная-сеть-материальной-группы)

**Краткий ответ:**
- **Угрозы:** кража оборудования, вирусы, перегрузка сети.
- **Каналы утечки:** общий доступ, Wi-Fi.
- **Причины потери целостности:** сбои маршрутизаторов, ошибки конфигурации.
- **Совместимость:** сканеры/принтеры, устаревшие драйверы.

**Подробный ответ:**
В сети материальной группы обычно есть складской учёт, документы о движении материалов. Потеря или искажение этих данных может повлиять на инвентаризацию и бюджеты. Надо контролировать физический доступ (**ключи**, **замки**) и логический (**пароль на Wi-Fi**, **VLAN**, **шифрование**). При сбое роутера или серверов складские операции могут встать. Также следует проверять совместимость оборудования (**принтеры этикеток**, **сканеры штрих-кодов**) с системами учёта.

---

## БИЛЕТ №6

### Вопрос 1. [Сохранение и откат рабочих версий системы](#билет-6-вопрос-1-сохранение-и-откат-рабочих-версий-системы)

**Краткий ответ:**
- Использование систем контроля версий (**Git**, **SVN**), создание snapshots (**virtual machine**), возможность быстро вернуться к предыдущему состоянию.

**Подробный ответ:**
Для сохранения рабочих версий системы часто используют системы контроля версий, такие как **Git** или **SVN**, которые позволяют отслеживать изменения, откатываться к предыдущим коммитам и работать в ветках. Также можно создавать snapshots виртуальных машин, чтобы при необходимости быстро восстановить систему до предыдущего состояния. Важно планировать **чекпоинты** — точки сохранения стабильных версий, поддерживать хранилище версии кода и конфигурации, а также иметь четкие процедуры на случай некорректного обновления.

### Вопрос 2. [Сохранение и восстановление баз данных](#билет-6-вопрос-2-сохранение-и-восстановление-баз-данных)

**Краткий ответ:**
- Регулярные выгрузки (**dumps**), настройка репликации, журналы транзакций (**point-in-time recovery**).

**Подробный ответ:**
Сохранение баз данных включает регулярные резервные копии через **mysqldump**, **pg_dump** для PostgreSQL или **MS SQL Maintenance Plans** для Microsoft SQL Server. Репликация данных на вторичные серверы обеспечивает дополнительную защиту. **Журналы транзакций** позволяют проводить восстановление до конкретного момента времени, что важно для минимизации потерь данных. Организация процесса включает автоматизацию бэкапов, хранение копий в разных локациях и регулярную проверку целостности резервных копий.

### Вопрос 3. [Анализ сопровождения объекта (одноранговая локальная сеть без выхода в Интернет)](#билет-6-вопрос-3-анализ-сопровождения-объекта-одноранговая-локальная-сеть-без-выхода-в-интернет)

**Краткий ответ:**
- **Угрозы:** физический доступ, флешки с вирусами.
- **Каналы утечки:** USB, локальные папки.
- **Причины нарушения целостности:** устаревшие ОС, отсутствие админ-учётки.
- **Совместимость:** новые программы не ставятся на **WinXP**.

**Подробный ответ:**
При отсутствии интернета снижается риск внешних сетевых атак, но возрастает риск внутренних (**недобросовестные сотрудники**, **флешки с вирусами**). Нужно внедрять антивирус, ограничивать права учётных записей, иметь офлайн-резервное копирование. Регулярные обновления ПО и ОС помогут предотвратить эксплуатацию известных уязвимостей. Также важно использовать групповые политики для ограничения доступа к критическим системам и данным.
...
### Вопрос 2. [Методы резервного копирования](#билет-7-вопрос-2-методы-резервного-копирования)

**Краткий ответ:**
- **Полное, дифференциальное, инкрементное, зеркалирование.**

**Подробный ответ:**
**Полное копирование** охватывает все данные, но требует много места. **Дифференциальное** хранит изменения, накопленные со времени последнего полного бэкапа. **Инкрементное** — только изменения со времени последнего любого бэкапа (полного или инкрементного). **Зеркалирование** (**RAID 1**, например) дублирует данные на другой носитель в реальном времени, позволяя быстро восстановиться при сбое, но не защищает от логических ошибок (например, если файл удалили случайно, это удаление также мгновенно «зазеркаливается»).
...
**Подробный ответ:**
Лёгкие модели (**Lean**, часть методологий **Agile**) позволяют стартовать без массивных технических заданий и регламентов, что хорошо для небольших или очень динамичных проектов. Это ускоряет реализацию, но может привести к пропуску важных рисков или сложностей, особенно если проект крупный. Тяжёлые модели предполагают подробные требования, обширное документирование, многоуровневое согласование. Они улучшают предсказуемость и контроль, но усложняют и удлиняют процесс.
...
**Подробный ответ:**
Обычно проект начинается с **подготовительной фазы** (формирование команды, первичное планирование). Далее идёт **анализ и проектирование** определяют структуру будущей системы (**техническая и бизнес-архитектура**). На **разработке** (или настройке, если это готовое решение) реализуются функции, затем всё **тестируется** (**от модульного до интеграционного и приёмочного**). После успешных тестов обучают пользователей. **Внедряют систему** в рабочую среду, где в первые дни идёт активная поддержка и исправление мелких сбоев. В ходе **эксплуатации** система работает под присмотром службы поддержки.
...
**Подробный ответ:**
Любая методология управления проектами (**PMBOK**) включает основные процессы: **инициация** (формулирование целей, назначение менеджера), **планирование** (составление план-графиков, бюджета, определение рисков, коммуникаций), **исполнение** (выполнение задач, координация), **контроль и мониторинг** (отслеживание прогресса, корректировка планов) и **закрытие** (подведение итогов, документирование уроков). **Риски** сопровождают проект на всех этапах: их нужно выявлять, оценивать и управлять ими, минимизируя негативное влияние. **Управление командой** — это назначение ролей, мотивация, разрешение конфликтов.
...
### Вопрос 3. [Создайте схему информационных систем](#билет-13-вопрос-3-создайте-схему-информационных-систем)
...
**Подробный ответ:**
**Big Bang** подразумевает одномоментную замену старой системы на новую. **Phased** — запуск отдельных модулей по очереди. **Parallel** — параллельное использование, позволяющее откатываться назад. **Pilot** — проверка системы в ограниченном масштабе. **Аутсорсинг** — привлечение внешних специалистов. Выбор подхода зависит от готовности организации к рискам, бюджета и культуры управления.

### Вопрос 3. [Представьте в виде таблицы преимущества и недостатки спиральной модели](#билет-19-вопрос-3-представьте-в-виде-таблицы-преимущества-и-недостатки-спиральной-модели)
...
**Подробный ответ:**
Под «живой» документацией понимают документы, которые не «умирают» после первичной разработки, а непрерывно поддерживаются в актуальном состоянии. Обычно такая документация ведётся в системах типа **Atlassian Confluence** или в **wiki-репозиториях** (**GitHub Wiki**), интегрированных с системой контроля версий. Это помогает всем участникам проекта иметь доступ к последним требованиям, описанию архитектуры и любым изменениям, включая исторические версии.
